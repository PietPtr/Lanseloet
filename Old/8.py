...
def startRunning():
    ...
    for i in range(0, 20):
        randomMapSlice = random.choice(mapSliceList)
        randomMapSlice.position = i * 128
        randomMapSlice.noBlock = 1
        mapSlices.append(randomMapSlice)
    playerY = 300
    playerX = 128
    lives = LIVES
"""font"""
...
class MapSlice(object):
    def __init__(self, position, noBlock, blockade): #blockade = list: [object, Y-position]
        self.position = position
...
"""Creates all possible mapSlices"""
#blockade = list: [object, Y-position]
for blockade in blockadeList:
    if blockade.height == 1:
        for yPos in range(2, 12):
            mapSliceList.append(MapSlice(0, 0, [blockade, yPos]))
    elif blockade.height == 2:
        for yPos in range(2, 11):
            mapSliceList.append(MapSlice(0, 0, [blockade, yPos]))
    ...
    if GameState == RUNPLAY:
        ...
        if len(mapSlices) < 16:
            if random.randint(0, 1) == 1:
                randomMapSlice = random.choice(mapSlices)
                randomMapSlice.position = len(mapSlices) * 128
                mapSlices.append(random.choice(mapSlices))
